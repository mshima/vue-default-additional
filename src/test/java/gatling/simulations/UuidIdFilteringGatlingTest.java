...
public class UuidIdFilteringGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all uuidIdFilterings").get("/api/uuid-id-filterings").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new uuidIdFiltering")
                        .post("/api/uuid-id-filterings")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_uuidIdFiltering_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created uuidIdFiltering").get("${new_uuidIdFiltering_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created uuidIdFiltering").delete("${new_uuidIdFiltering_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UuidIdFiltering entity").exec(scn);
