...
public class EntityUuidIdGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all entityUuidIds").get("/api/entity-uuid-ids").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityUuidId")
                        .post("/api/entity-uuid-ids")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityUuidId_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created entityUuidId").get("${new_entityUuidId_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created entityUuidId").delete("${new_entityUuidId_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityUuidId entity").exec(scn);
