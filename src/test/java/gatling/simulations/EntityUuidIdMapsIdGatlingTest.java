...
public class EntityUuidIdMapsIdGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityUuidIdMapsIds")
                    .get("/api/entity-uuid-id-maps-ids")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityUuidIdMapsId")
                        .post("/api/entity-uuid-id-maps-ids")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityUuidIdMapsId_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityUuidIdMapsId").get("${new_entityUuidIdMapsId_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created entityUuidIdMapsId").delete("${new_entityUuidIdMapsId_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityUuidIdMapsId entity").exec(scn);
