...
/**
 * Performance test for the EntityCustomIdRequiredDTOMapsId entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityCustomIdRequiredDTOMapsIdGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityCustomIdRequiredDTOMapsIds")
                    .get("/api/entity-custom-id-required-dto-maps-ids")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityCustomIdRequiredDTOMapsId")
                        .post("/api/entity-custom-id-required-dto-maps-ids")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityCustomIdRequiredDTOMapsId_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityCustomIdRequiredDTOMapsId")
                            .get("${new_entityCustomIdRequiredDTOMapsId_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityCustomIdRequiredDTOMapsId")
                        .delete("${new_entityCustomIdRequiredDTOMapsId_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityCustomIdRequiredDTOMapsId entity").exec(scn);
