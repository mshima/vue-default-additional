...
/**
 * Performance test for the EntityCustomIdRequiredDTORel entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityCustomIdRequiredDTORelGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityCustomIdRequiredDTORels")
                    .get("/api/entity-custom-id-required-dto-rels")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityCustomIdRequiredDTORel")
                        .post("/api/entity-custom-id-required-dto-rels")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityCustomIdRequiredDTORel_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityCustomIdRequiredDTORel")
                            .get("${new_entityCustomIdRequiredDTORel_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityCustomIdRequiredDTORel")
                        .delete("${new_entityCustomIdRequiredDTORel_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityCustomIdRequiredDTORel entity").exec(scn);
