...
/**
 * Performance test for the EntityUuidIdDTOMapsId entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class EntityUuidIdDTOMapsIdGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityUuidIdDTOMapsIds")
                    .get("/api/entity-uuid-id-dto-maps-ids")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityUuidIdDTOMapsId")
                        .post("/api/entity-uuid-id-dto-maps-ids")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityUuidIdDTOMapsId_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityUuidIdDTOMapsId").get("${new_entityUuidIdDTOMapsId_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created entityUuidIdDTOMapsId")
                        .delete("${new_entityUuidIdDTOMapsId_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityUuidIdDTOMapsId entity").exec(scn);
