...
/**
 * Performance test for the UuidIdFilteringRelationship entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class UuidIdFilteringRelationshipGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all uuidIdFilteringRelationships")
                    .get("/api/uuid-id-filtering-relationships")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new uuidIdFilteringRelationship")
                        .post("/api/uuid-id-filtering-relationships")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_uuidIdFilteringRelationship_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created uuidIdFilteringRelationship")
                            .get("${new_uuidIdFilteringRelationship_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created uuidIdFilteringRelationship")
                        .delete("${new_uuidIdFilteringRelationship_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UuidIdFilteringRelationship entity").exec(scn);
