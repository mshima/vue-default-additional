...
public class EntityUuidIdDTORelGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all entityUuidIdDTORels")
                    .get("/api/entity-uuid-id-dto-rels")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new entityUuidIdDTORel")
                        .post("/api/entity-uuid-id-dto-rels")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_entityUuidIdDTORel_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created entityUuidIdDTORel").get("${new_entityUuidIdDTORel_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created entityUuidIdDTORel").delete("${new_entityUuidIdDTORel_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the EntityUuidIdDTORel entity").exec(scn);
