import { vitest } from 'vitest';
import { type MountingOptions, shallowMount } from '@vue/test-utils';
import sinon, { type SinonStubbedInstance } from 'sinon';
import { type RouteLocation } from 'vue-router';

import EntityCustomIdRequiredDTORelDetails from './entity-custom-id-required-dto-rel-details.vue';
import EntityCustomIdRequiredDTORelService from './entity-custom-id-required-dto-rel.service';
import AlertService from '@/shared/alert/alert.service';

type EntityCustomIdRequiredDTORelDetailsComponentType = InstanceType<typeof EntityCustomIdRequiredDTORelDetails>;

let route: Partial<RouteLocation>;
const routerGoMock = vitest.fn();

vitest.mock('vue-router', () => ({
  useRoute: () => route,
  useRouter: () => ({ go: routerGoMock }),
}));

const entityCustomIdRequiredDTORelSample = { relatedId: 123 };

describe('Component Tests', () => {
  let alertService: AlertService;

  afterEach(() => {
    vitest.resetAllMocks();
  });

  describe('EntityCustomIdRequiredDTORel Management Detail Component', () => {
    let entityCustomIdRequiredDTORelServiceStub: SinonStubbedInstance<EntityCustomIdRequiredDTORelService>;
    let mountOptions: MountingOptions<EntityCustomIdRequiredDTORelDetailsComponentType>['global'];

    beforeEach(() => {
      route = {};
      entityCustomIdRequiredDTORelServiceStub =
        sinon.createStubInstance<EntityCustomIdRequiredDTORelService>(EntityCustomIdRequiredDTORelService);

      alertService = new AlertService({
        i18n: { t: vitest.fn() } as any,
        toastController: {
          show: vitest.fn(),
        } as any,
      });

      mountOptions = {
        stubs: {
          'font-awesome-icon': true,
          'router-link': true,
        },
        provide: {
          alertService,
          entityCustomIdRequiredDTORelService: () => entityCustomIdRequiredDTORelServiceStub,
        },
      };
    });

    describe('Navigate to details', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        entityCustomIdRequiredDTORelServiceStub.find.resolves(entityCustomIdRequiredDTORelSample);
        route = {
          params: {
            entityCustomIdRequiredDTORelId: `${123}`,
          },
        };
        const wrapper = shallowMount(EntityCustomIdRequiredDTORelDetails, { global: mountOptions });
        const comp = wrapper.vm;
        // WHEN
        await comp.$nextTick();

        // THEN
        expect(comp.entityCustomIdRequiredDTORel).toMatchObject(entityCustomIdRequiredDTORelSample);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        entityCustomIdRequiredDTORelServiceStub.find.resolves(entityCustomIdRequiredDTORelSample);
        const wrapper = shallowMount(EntityCustomIdRequiredDTORelDetails, { global: mountOptions });
        const comp = wrapper.vm;
        await comp.$nextTick();

        comp.previousState();
        await comp.$nextTick();

        expect(routerGoMock).toHaveBeenCalledWith(-1);
      });
    });
  });
});
